...
async function db_verifyUser(username, password, cb) {
  ...
  try {
    ...
    const db_resp = await knex('Users_Auth').join('Users', 'Users_Auth.user_id', 'Users.id').where('Users.email', email).select('Users_Auth.salt AS salt', 'Users_Auth.hash AS hash', 'Users_Auth.user_id AS uid').limit(1).catch(db_err => { if (db_err) err_code = db_err.code; });
